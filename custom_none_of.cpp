    ...
    for (auto size : sizes) {
        ...
        for (auto num_threads : test_threads) {
            ...
            timeit([&test_data, pred, num_threads]() {
                custom_none_of(test_data.begin(), test_data.end(), pred, num_threads);
            }, 3);
        ...
        cout << "Best performance with " << best_threads << " threads (" << best_time << " ms)" << endl;
        cout << "Ratio to hardware threads: " << static_cast<double>(best_threads) / hw_threads << endl;
    }
}